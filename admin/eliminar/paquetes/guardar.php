...
if($token==""){
    ...
    try {
		...
		if(empty($txtnombre) || empty($txtprecio) || empty($txtporcentaje) || empty($txtimg) || empty($txtmeses) || $txtmeses < 1){
            ...
            if($fila){
                ...
                // Guardar la imagen y verificar
                 // Guardar la imagen y verificar
            	if(!(move_uploaded_file($_FILES['txtimg']['tmp_name'],$path. 'paquetes_imagenes/'. $nombre_archivo))){
                ...
                $sql="CALL CrearPaquetesAdmin( '$txtnombre', '$txtprecio', '$txtporcentaje', '$nombre_archivo', '$txtdescripcion', $txtmeses)";
                $conexion=conexion();
